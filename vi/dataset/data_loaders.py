...
class DataLoaders:
    ...
    def __call__(self, shuffle=False, indices=None):
        ...
        return DataLoaderWrapper(self.dataset, use_cuda=self.use_cuda, sampler=sampler,
                                 **self.kwargs)
...
class TrainTestDataLoaders(DataLoaders):
    ...
    def __init__(self, dataset, train_size=0.1, test_size=None, seed=0, **kwargs):
        """
        :param train_size: float, int, or None (default is 0.1)
        :param test_size: float, int, or None (default is None)
        """
        super(TrainTestDataLoaders, self).__init__(dataset, **kwargs)
...
class SemiSupervisedDataLoaders(DataLoaders):
    ...
    def __init__(self, dataset, n_labelled_samples_per_class=50, n_label_array=None, seed=0, use_cuda=True, **kwargs):
        """
        :param n_labelled_samples_per_class: number of labelled samples per class
        """
        super(SemiSupervisedDataLoaders, self).__init__(dataset, use_cuda=use_cuda, **kwargs)
        if n_label_array is None:
            n_labelled_samples_per_class_array = [n_labelled_samples_per_class] * dataset.n_labels
        else:
            n_labelled_samples_per_class_array = n_label_array
        labels = np.array(dataset.labels).ravel()
        np.random.seed(seed=seed)
